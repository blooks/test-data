...
var deleteTransfers = function (callback) {
    mongo.db.collection('transfers').remove({}, function(err){
        return callback(err);
    });
};
var deleteAddresses = function (callback) {
    mongo.db.collection('bitcoinaddresses').remove({}, function(err){
        return callback(err);
    });
};
var deleteWallets = function (callback) {
    mongo.db.collection('bitcoinwallets').remove({}, function(err){
        return callback(err);
    });
};
var deleteExchangeRates = function (callback) {
    mongo.db.collection('exchangeRates').remove({}, function(err){
        return callback(err);
    });
};
