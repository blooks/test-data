...
var BIP32Wallet = require('coyno-wallets').BIP32Wallet;
var SingleAddressesWallet = require('coyno-wallets').SingleAddressesWallet;
 ...
 * @returns {boolean}
 */
var checkDataTypes = function(dataTypes) {
...
var insertWallet = function(callback) {
    log('Inserting Wallets',bip32wallet,singleAddressesWallet);
    mongo.db.collection('bitcoinwallets').insertMany([bip32wallet,singleAddressesWallet], callback);
};
...
var insertExchanges = function(callback) {
    log('Inserting Exchange');
    var exchangesArray = _.toArray(exchanges);
    mongo.db.collection('exchanges').insertMany(exchangesArray, callback);
};
var insertAddresses = function(callback) {
    log('Inserting Addresses');
    mongo.db.collection('bitcoinaddresses').insert(addressWithLotsOfTx, callback);
};
var insertTransfers = function(callback) {
    log('Inserting Transfers');
    mongo.db.collection('transfers').insert(disconnectedBitcoinTransfers, callback);
};
...
var getFillFunctions = function (toBeMockedUp) {
    var result = [];
    if (!checkDataTypes(toBeMockedUp)) {
        log('Throwing error for trying to fill db for unsupported datatype.');
    ...
    if (toBeMockedUp.indexOf('exchanges')>-1) {
        result.push(insertExchanges);
    }
    if (toBeMockedUp.indexOf('addresses')>-1) {
        result.push(insertAddresses);
    }
    if (toBeMockedUp.indexOf('transfers')>-1) {
        result.push(insertTransfers);
    }
    return result;
};
...
testDataManager.prototype.getWallet = function(type) {
    switch (type) {
        ...
        case 'single-addresses':
            return new SingleAddressesWallet(singleAddressesWallet);
    }
